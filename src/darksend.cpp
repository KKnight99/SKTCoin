    ...
    if(state == POOL_STATUS_FINALIZE_TRANSACTION) {
      ...
      for(unsigned int i = 0; i < entries.size(); i++){
            ...
            txNew.vout.push_back(v);
                ...
                std::random_shuffle ( txNew.vout.begin(), txNew.vout.end(), randomizeList); //sktdev
    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat SKT and make it unusable. To
        ...
        if(nUseQueue > 33){
            ...
            BOOST_FOREACH(CDarksendQueue& dsq, vecDarksendQueue){
                ...
                BOOST_FOREACH(CTxIn usedVin, vecMasternodesUsed){
                    if(dsq.vin == usedVin) {
                        fUsed = true; // SKTDev 08.05.2015 master (#559)
                        break;
                    }
                }
				if(fUsed) continue;
                ...
                if(ConnectNode((CAddress)addr, NULL, true)){
                    ...
                    dsq.time = 0; //remove node
                    continue; // SKTDev 08.05.2015 master (#559)
                }
            }
        }
