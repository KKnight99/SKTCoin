...
#if defined(NDEBUG)
# error "SKT cannot be compiled without assertions."
#endif
...
int64_t CTransaction::nMinTxFee = NMINTXFEE;  // Override with -mintxfee // 12-05-2015 sktdev old 10000 dash 
...
const string strMessageMagic = "SKT Signed Message:\n";
...
int64_t GetBlockValue(int nBits, int nHeight, int64_t nFees)
{
   /*double dDiff = (double)0x0000ffff / (double)(nBits & 0x00ffffff);*/
	...
	if (nHeight == 1)
		...
		// SKTdev and SKT Support have no Premine Coins
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	unsigned int retarget = DIFF_NULL;
		...
		if (TestNet()){
        	if (pindexLast->nHeight + 1 >= 3) { retarget = DIFF_DKGW3; } 
        else{ retarget = DIFF_BTC;}
        }
	 ...
	 // SKT: This fixes an issue where a 51% attack can change difficulty at will.
...
// OLD BTC Function END
        ...
        {	
            static const uint64_t blocksTargetSpacing = 5 * 60; // 5 minutes
            static const unsigned int timeDaySeconds = 60 * 60 * 24;
            uint64_t pastSecondsMin = timeDaySeconds * 0.5; // Old 0,025
            uint64_t pastSecondsMax = timeDaySeconds * 7;
            uint64_t pastBlocksMin = pastSecondsMin / blocksTargetSpacing;
            uint64_t pastBlocksMax = pastSecondsMax / blocksTargetSpacing;
            return KimotoGravityWell(pindexLast, pblock, blocksTargetSpacing, pastBlocksMin, pastBlocksMax);
        ...
        else if (retarget == DIFF_DELTA || retarget == DIFF_NULL )
        {
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
   ...
   // ----------- masternode payments -----------
    ...
    if(!IsSporkActive(SPORK_1_MASTERNODE_PAYMENTS_ENFORCEMENT)){
        MasternodePayments = true; // SKTdev
    ...
    if(MasternodePayments)
    {
        ...
        if(pindex != NULL){
            if(pindex->GetBlockHash() == block.hashPrevBlock){
				...
				//SKTdev 18-10-2015 SKT proof of payment Number 2
...
bool AcceptBlock(CBlock& block, CValidationState& state, CDiskBlockPos* dbp)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        }*/ //sktdev Prüfunktion 25-05-20105
