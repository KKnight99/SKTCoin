...
unsigned int static DUAL_KGW3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
	...
	//Fusion from Retarget and Classic KGW3 (SKT=)
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, Dash - DarkGravity v3, written by Evan Duffield - evan@sktpay.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64_t TargetBlocksSpacingSeconds, uint64_t PastBlocksMin, uint64_t PastBlocksMax) {
        ...
        const CBlockHeader *BlockCreating = pblock;// SKTdev add from old KGW
        BlockCreating = BlockCreating; //SKTdev add from old KGW
        uint64_t PastBlocksMass = 0;
        int64_t PastRateActualSeconds = 0;
        int64_t PastRateTargetSeconds = 0;
        double PastRateAdjustmentRatio = double(1);
	...
	int64_t LatestBlockTime = BlockLastSolved->GetBlockTime();
        for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
                ...
                PastDifficultyAveragePrev = PastDifficultyAverage;
                 if (LatestBlockTime < BlockReading->GetBlockTime()) //Fix SKTdev
                 {
                 	LatestBlockTime = BlockReading->GetBlockTime();
                 }

                PastRateActualSeconds = LatestBlockTime - BlockReading->GetBlockTime(); //Fix SKTdev
            ...
            if (PastRateActualSeconds < 15) { PastRateActualSeconds = 15; }
                ...
                // else { if (PastRateActualSeconds < 0) { PastRateActualSeconds = 0; }; } //Fix SKTdev old KGW
                if (PastRateActualSeconds != 0 && PastRateTargetSeconds != 0) {
                PastRateAdjustmentRatio = double(PastRateTargetSeconds) / double(PastRateActualSeconds);
                }
                // KGW3
                ...
                // KGW 3
                // EventHorizonDeviation = 1 + (0.7084 * pow((double(PastBlocksMass)/double(144)), -1.228)); // Fix SKTdev
                EventHorizonDeviationFast = EventHorizonDeviation;
                EventHorizonDeviationSlow = 1 / EventHorizonDeviation;
