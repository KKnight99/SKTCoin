...
void ParseParameters(int argc, const char* const argv[])
{
    ...
    BOOST_FOREACH(const PAIRTYPE(string,string)& entry, mapArgs)
    {
        ...
        if (name.find("--") == 0)
        {
            std::string singleSKT(name.begin()+1, name.end());
            if (mapArgs.count(singleSKT) == 0)
                mapArgs[singleSKT] = entry.second;
            name = singleSKT;
        }
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SKT
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SKT
    // Mac: ~/Library/Application Support/SKT
    // Unix: ~/.skt
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SKT";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "skt.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty skt.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override skt.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "sktd.pid"));
...
void AddTimeData(const CNetAddr& ip, int64_t nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        // Old (abs64(nMedian) < 70 * 60) SKTdev 31-05-2015
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong SKT will not work properly.");
